  ...
  <div >
    <Tabs :animated="false">
      <Tab-pane label="安全隐患录入">
        <form class="form1" name="myform" method="post" enctype="multipart/form-data" action="http://10.120.2.110:8080/SafeCDTU/workOfEvent/addEvent" @submit="check">
          ...
          <div class="margin">
            <label> 隐患区域： </label>
            <label v-for="item in areas"><input name="areId" type="radio" :value="item.areId" />{{item.name}} <input   name="parAreaId"  :value="item.parAreaId" hidden/> </label></label>
  ...
  created() {
    //获取登陆用户id
     ...
     this.$http.post(//连接ei后端测试
                  ROUTE.toReplace+'/workOfEvent/getOwnArea',
                  {
