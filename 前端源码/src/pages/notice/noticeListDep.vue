<template>
  <div>
    <div>
      <p style="font-size:20px;margin-bottom: 20px;">位置：部门通知</p>
    ...
    <div v-if="showlist">
        ...
        <li class="li" v-for="item in tabledata" @dblclick="getOneContent(item)">
          ...
          <span class="li3">
            <Button type="primary" size="small"  @click="getOneContent(item)">查看</Button>
            <Button type="error" size="small" v-if="showdel" @click="del(item)">删除</Button>
          </span>
        </li>
...
export default {
  ...
  methods: {
    ...
    maketable1() {
      ...
      //datas为数据库导出的,这里为自定义
      this.$http.post(
        ROUTE.toReplace+'/context/getContent',
        {
        ...
        .then(function (res) {
          ...
          this.tabledata = res.data.contextList
        })
    ...
    searchcontext() {
      ...
      this.current=1
      this.$http.post(
        ROUTE.toReplace+'/context/searchContext',
        {
    ...
    getOneContent(item) {
       this.$http.post(
        ROUTE.toReplace+'/context/seeContent',
        {
          txtId: item.txtId,
        },
        { emulateJSON: true }
    ...
    del(item) {
      var statu;
      if (confirm("确定删除吗？")) {
        this.$http.post(ROUTE.toReplace+'/context/removeOneContent',
          {
            txtId: item.txtId
