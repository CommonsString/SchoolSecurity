<template>
  <div>
    <div>
      <p style="font-size:20px;margin-bottom: 20px;">位置：我的通知</p>
    ...
    <div v-if="showlist">
      <ul class="ul">
        ...
        <li class="li" v-for="item in tabledata" @dblclick="getOneContent(item)">
          ...
          <span class="li3">
            <Button type="primary" size="small" @click="getOneContent(item)">{{ item.isSee }}</Button>
          </span>
        </li>
...
export default {
  ...
  methods: {
    ...
    maketable1() {
      ...
      this.$http.post(
        ROUTE.toReplace+'/context/getOwnContent',
        {
    ...
    searchcontext() {
      ...
      this.current = 1
      this.$http.post(
        ROUTE.toReplace+'/context/searchOwnContent',
        {
