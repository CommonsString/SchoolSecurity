...
export default {
  ...
  methods: {
    ...
    toEdit(){
      ...
      this.operation='基本信息查看'
      this.clearForm()
      this.loadArea()//重置组件加载管理区域
    },
    clearForm(){
    ...
    resetForm(){
      ...
      // 18/4/2 ////////////////////////////
      // 解决点编辑后不选择负责人时
      // 区域负责人formData.uId参数为空并上传更改的问题
      this.formData.uId=this.currentArea.uId
      //////////////////////////////////////////
    ...
    removeArea(){
      ...
      if(confirm('确认删除区域：'+this.currentArea.name+'?')){
        this.submit()
      }
      else{
        this.cancelEdit()
    ...
    submit(type){
      ...
      // 18/3/24 添加 //////////////////////////////////
      if(this.operationMethod=='addSubArea'){
        this.formData.parAreaId=this.currentArea.areId
      }
      else{
        this.formData.parAreaId = this.currentArea.parAreaId
      }
      //////////////////////////////////////////////////
      this.$http.post(
        ROUTE.toReplace+'/infoSearchAndFix/'+this.operationMethod,
        this.formData,
        {emulateJSON: true}
      )
      .then(function(res){
        if(res.data.state==true){
          //使对应节点做相应操作，重新查询数据
          this.currentArea.it.toReload(this.operation)
    ...
    loadArea(){
      ...
      this.$http.post(
        ROUTE.toReplace+'/infoSearchAndFix/'+method,
        {
    ...
    // 此处已更改为将所有用户都返回到列表中选择///////19/3/20注
    findSubUser(areId){
      this.$http.post(
        ROUTE.toReplace+'/infoSearchAndFix/findSubUser',
        {
          // uId: adminId,
    ...
    //   this.$http.post(
    //     'api/infoSearchAndFix/findSubUser',
    //     {
    //       uId: adminId,
    ...
    getRule(){
      ...
      this.$http.post(
        ROUTE.toReplace+'/infoSearchAndFix/getRule',
        {areId: this.currentArea.areId},
        {emulateJSON: true}
      )
      .then(function(res){
    ...
    linkTo(){
      let href = ROUTE.location+"/editRule.jsp?FSEdfsfKfsefseFGfsfNESGfsefNfseFEJS="+this.userInfo.uId
